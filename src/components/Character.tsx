...
class Character extends Component<any, any> {
    ...
    fetchCharacterData = async () => {
        ...
        const locationDetails = await RickMortyService.fetchLocationDetails(location.url);
        const originDetails = await RickMortyService.fetchLocationDetails(origin.url);

        const episodes = await RickMortyService.fetchEpisodesDetails(RickMortyUtils.getIdsfromUrls(episode));
        ...
        return (
            <>
                <Card variant="outlined">
                    <CardContent>
                        <Avatar alt={data.name} src={data.image} sx={{ width: 56, height: 56 }} />
                        <h1>{data.name}</h1>
                        <p>{data.species}</p>
                        <p>{data.gender}</p>
                        <p>{data.status}</p>
